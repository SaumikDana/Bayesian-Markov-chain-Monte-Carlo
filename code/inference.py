...
def pylith_gprs_inference(self):
    # load objects!!!
    model_lstm = load_object(self.file1)  # ROM
    u_appended = load_object(self.file2) # noisy data
    ...
    for ii in range(0,num_p):
        ...
        u = u_appended[ii*num_tsteps:ii*num_tsteps+num_tsteps,0]
        ...
        problem_type = 'rom'
        MCMCobj2=MCMC(p_,qpriors=qpriors,nsamples=nsamples,nburn=nburn,data=acc,problem_type=problem_type,lstm_model=model_lstm,qstart=qstart,adapt_interval=10,verbose=True)
        qparams2=MCMCobj2.sample() # run the Bayesian/MCMC algorithm
        std_MCMC2 = MCMCobj2.std2
        MCMCobj2.plot_dist(qparams2,'reduced order',dc)
